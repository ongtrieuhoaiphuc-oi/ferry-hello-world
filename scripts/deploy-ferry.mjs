...
const stateDir = join(process.env.RUNNER_TEMP || tmpdir(), 'omniroute-direct');
...
const log = (message) => console.log(`\n\x1b[36m[omniroute]\x1b[0m ${message}`);
...
const app = {
  name: cfg.OMNIROUTE_APP || 'omiroute',
  host: cfg.OMNIROUTE_HOSTNAME || `${cfg.OMNIROUTE_HOST_PREFIX || 'omiroute'}.${cfg.DOKKU_HOSTNAME}`,
  port: Number(cfg.OMNIROUTE_PORT || 20128),
  memory: Number(cfg.OMNIROUTE_MEMORY || 1024),
  image: 'omniroute-ci:cached',
};
...
async function resolveZone(hostname, accountId) {
  ...
  fail(`No zone for ${hostname}`);
}
const dokku = (args) => run('docker', ['exec', 'dokku', 'dokku', ...args]);
const dokkuOutput = (args) => output('docker', ['exec', 'dokku', 'dokku', ...args]);

async function main() {
  mkdirSync(stateDir, { recursive: true });
  const accounts = cfg.CF_ACCOUNT_ID ? [{ id: cfg.CF_ACCOUNT_ID }] : await cf('GET', '/accounts?per_page=50');
  const accountId = accounts[0]?.id || fail('No Cloudflare account');
  ...
  const token = await cf('GET', `/accounts/${accountId}/cfd_tunnel/${tunnel.id}/token`);
  ...
  const dns = { type: 'CNAME', name: app.host, content: `${tunnel.id}.cfargotunnel.com`, proxied: true, ttl: 1 };
  if (records[0]?.id) await cf('PUT', `/zones/${zoneId}/dns_records/${records[0].id}`, dns);
  else await cf('POST', `/zones/${zoneId}/dns_records`, dns);
  ...
  const existing = new Set(dokkuOutput(['apps:list']).split(/\r?\n/).map((line) => line.trim()));
  if (!existing.has(app.name)) dokku(['apps:create', app.name]);
  ...
  log('Releasing OmniRoute');
  dokku(['git:from-image', app.name, app.image]);

  await cf('PUT', `/accounts/${accountId}/cfd_tunnel/${tunnel.id}/configurations`, { config: { ingress: [{ hostname: app.host, service: 'http://dokku:80' }, { service: 'http_status:404' }] } });
  run('docker', ['compose', '--env-file', envFile, '-f', composeFile, 'restart', 'cloudflared']);
  const url = `https://${app.host}/api/monitoring/health`;
  for (let i = 1; i <= 120; i += 1) {
    try {
      const response = await fetch(url, { signal: AbortSignal.timeout(10000) });
      if (response.ok) {
        log(`Live: https://${app.host}`);
    ...
    if (i % 12 === 0) log(`Still waiting for ${url}`);
    await sleep(5000);
  }
